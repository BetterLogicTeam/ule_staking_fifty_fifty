...
import { getRegRoiIncomeReport } from "../../store/actions/dailyYield";
import moment from "moment";
import { CustomTable } from "../../components/CustomTable";

export const RegRoiIncome = () => {
  ...
  const getAllData = () => {
    if (user) {
      ...
      dispatch(getRegRoiIncomeReport(uId));
    }
  };
  useEffect(() => {
    getAllData();
  ...
  useEffect(() => {
    ...
    bonusDyReport.forEach((item, index) => {
      arr.push({
        ...
        date:moment(item?.date.replace("Z",'')).format("M/D/YYYY"),
      });
    });
    setDateState([...arr]);
  }, [bonusDyReport]);
      ...
      <div class="content-wrapper">
        <div class="grid grid-1">
          <div class="section-heading">
            <h2>Registration Daily Yield</h2>
          </div>
          <CustomTable
            columns={[
